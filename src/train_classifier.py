...
from tensorflow.python.platform import gfile  # type: ignore
...
def main(input_directory,
         ...
         is_train=True):
    """
    Loads images from :param input_dir,
    creates embeddings using a model defined at :param model_path, and trains
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        images_placeholder = tf.get_default_graph().get_tensor_by_name(
            "input:0")
        embedding_layer = tf.get_default_graph().get_tensor_by_name(
            "embeddings:0")
...
def _get_test_and_train_set(input_dir,
                            min_num_images_per_label,
                            split_ratio=0.7):
    """
    Load train and test dataset. Classes with <
    :param min_num_images_per_label will be filtered out.
    :param input_dir:
...
def _load_images_and_labels(dataset,
                            image_size,
                            batch_size,
                            num_threads,
                            num_epochs,
...
def _create_embeddings(embedding_layer, images, labels, images_placeholder,
    ...
    try:
        ...
        while True:
            ...
            logger.info('Processing iteration {} batch of size: {}'.format(
                i, len(batch_labels)))
            emb = sess.run(
                embedding_layer,
                feed_dict={
            ...
            emb_array = np.concatenate([emb_array,
                                        emb]) if emb_array is not None else emb
            label_array = np.concatenate(
                [label_array,
                 batch_labels]) if label_array is not None else batch_labels
            i += 1
...
def _evaluate_classifier(emb_array, label_array, classifier_filename):
    ...
    if not os.path.exists(classifier_filename):
        raise ValueError(
            'Pickled classifier not found, have you trained first?')
